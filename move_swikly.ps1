...
$filter = "@SQL=" + $q + "urn:schemas:httpmail:fromemail" + $q + " LIKE " + $sq + "%swikly%" + $sq
$items = @($inbox.Items.Restrict($filter))
if ($items.Count -eq 0) { Write-Host "0 Mails gefunden"; exit 0 }
foreach ($item in $items) { try { $item.Move($dest) | Out-Null } catch {} }
Write-Host ("{0} Mails  ->  Persoenlich/Urlaub und Reise  [Swikly]" -f $items.Count) -ForegroundColor Green
